...
import { PrismaClient, RoomRole, ParticipantStatus } from '@prisma/client';
...
redis?.on('error', (error: Error) => console.error('Redis:', error.message));
redis?.connect().catch((error: Error) => console.warn('Redis unavailable:', error.message));
...
io.on('connection', socket => {
  ...
  socket.on('room:moderate', async ({ roomId, targetUserId, action }, ack) => { try { const host = await roomMember(roomId, socket.data.userId); if (host?.role !== 'HOST') throw Error('Only the host can moderate'); const target = await roomMember(roomId, targetUserId); if (!target || target.status !== 'ACTIVE') throw Error('Participant is not active'); const data = action === 'promote' ? { role: RoomRole.SPEAKER, handRaised: false } : action === 'demote' ? { role: RoomRole.LISTENER, isMuted: false } : action === 'mute' ? { isMuted: true } : action === 'unmute' ? { isMuted: false } : { status: ParticipantStatus.REMOVED, leftAt: new Date(), handRaised: false }; const participant = await prisma.roomParticipant.update({ where: { id: target.id }, data, include: { user: { select: userSelect } } }); io.to(roomId).emit('room:moderated', { action, participant }); if (action === 'remove') io.to(roomId).emit('room:user_removed', { userId: targetUserId }); ack?.({ ok: true, participant }); } catch (error) { ack?.({ ok: false, error: error instanceof Error ? error.message : 'Moderation failed' }); } });
